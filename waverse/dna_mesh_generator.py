...
class DNAMeshGenerator:
    ...
    def _generate_tree(self, p: Dict):
        ...
        if trunk_segments and len(trunk_segments) > 0:
            for i, seg in enumerate(trunk_segments[:4]):  # Limit segments
                ...
                self._draw_trunk_segment(
                    current_y, seg_length, current_width,
                    seg_taper, seg_curve, seg_twist + p['spiral_factor'] * 0.3 * i,
                    p['trunk_color']
                )
    ...
    def _draw_trunk_segment(self, start_y: float, height: float, width: float,
                            taper: float, curve: float, twist: float, color: Color):
        """Draw a trunk segment with curve and twist."""
        segments = 6
        rings = 4
        ...
        for ring in range(rings):
            t1 = ring / rings
            t2 = (ring + 1) / rings
            y1 = start_y + t1 * height
            y2 = start_y + t2 * height
            
            w1 = width * (1 - t1 * (1 - taper))
            w2 = width * (1 - t2 * (1 - taper))
            
            # Curve offset
            offset_x1 = curve * math.sin(t1 * math.pi) * height * 0.15
            offset_x2 = curve * math.sin(t2 * math.pi) * height * 0.15
            
            twist1 = twist * t1 * math.pi
            twist2 = twist * t2 * math.pi
            
            for seg in range(segments):
                a1 = (seg / segments) * 2 * math.pi + twist1
                a2 = ((seg + 1) / segments) * 2 * math.pi + twist1
                a1_next = (seg / segments) * 2 * math.pi + twist2
                a2_next = ((seg + 1) / segments) * 2 * math.pi + twist2
                
                self._add_quad(
                    (math.cos(a1) * w1 + offset_x1, y1, math.sin(a1) * w1),
                    (math.cos(a2) * w1 + offset_x1, y1, math.sin(a2) * w1),
                    (math.cos(a2_next) * w2 + offset_x2, y2, math.sin(a2_next) * w2),
                    (math.cos(a1_next) * w2 + offset_x2, y2, math.sin(a1_next) * w2),
                    color
                )
